...
public class BasicRideController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<RideResponseDto> getOne(@PathVariable String id) {
        RideResponseDto ride = rideBasicService.getOne(id);
        return ResponseEntity.ok(ride);
    }

    @PostMapping
    public ResponseEntity<RideResponseDto> create(@Valid @RequestBody RideRequestDto rideDto) {
